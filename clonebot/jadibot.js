...
const jadibot = async (BRIGHTZ, m, from) => {
      ...
      client[from].ev.on("connection.update", async up => {
        ...
        if (connection === "close") {
          if (reason === DisconnectReason.restartRequired) {
            ...
            return BRIGHTZ.sendMessage(from, { text: "Anda sudah tidak lagi menjadi bot." })
          }
        }
      })
      ...
      client[from].ev.process(async (events) => {
        if (events['messages.upsert']) {
          ...
          for (let msg of upsert.messages) {
            ...
            require("../BRIGHTZ")(client[from], m, store)
          }
        }
      })
...
async function stopjadibot(BRIGHTZ, from) {
  if (!Object.keys(client).includes(from)) {
    return BRIGHTZ.sendMessage(from, { text: "Anda tidak ada di list jadi bot." })
  }
  try {
...
async function listjadibot(BRIGHTZ, m) {
  ...
  return BRIGHTZ.sendMessage(from, { text: text.trim(), mentions, })
}
